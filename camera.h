...
class camera {
    public:
        ...
        point3 lookfrom = point3(0, 0, 0);   // Point that the camera is looking from
        point3 lookat   = point3(0, 0, -1);  // Point that the camera is looking to (may also use a direction instead)
    ...
    private:
        ...
        vec3 u, v, w;                // Camera frame basis vectors (for arbitrary camera position)
        ...
        void initialize() {
            ...
            // Calculate u, v, and w unit bases vectors for the camera coordinate frame (orthonormal basis)
